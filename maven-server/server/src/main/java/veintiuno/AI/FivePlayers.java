    ...
    protected static int getMove(Stack<Integer> gameStack, int moves) {
        ...
        if (stackSize == 6){
            ...
            if ((gameStack.get(stackSize - 3) + gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1) >= 21)
                && (gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1) >= 8)
                && (gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1) < 17)) {
            return 17 - (gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1));
            // [5] + [6] = 17 in vollem Stack
            }
        }
