    ...
    protected static int getMove(Stack<Integer> gameStack, int moves) {
        ...
        if (stackSize == 6){
            ...
            if ((gameStack.get(stackSize - 3) + gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1) >= 21)
                && (gameStack.get(gameStack.size() - 2) + gameStack.get(stackSize - 1) >= 9)
                && (gameStack.get(gameStack.size() - 2) + gameStack.get(stackSize - 1) < 18)) {
                    return 18 - (gameStack.get(stackSize - 2) + gameStack.get(stackSize - 1));
            }       // [5] + [6] = 18 in vollem Stack
        }
